...
function findVenta_by_id($table,$id)
{
  global $db;
  $id = (int)$id;
    if(tableExists($table)){
...
function deleteVenta_by_id($table,$id)
{
  global $db;
  if(tableExists($table))
   {
    $sql = "DELETE FROM ".$db->escape($table);
    $sql .= " WHERE idVenta=". $db->escape($id);
    $sql .= " LIMIT 1";
    $db->query($sql);
    return ($db->affected_rows() === 1) ? true : false;
   }
...
/*--------------------------------------------------------------*/
 ...
 function updateLastLogIn($user_id)
  ...
  function update_product_qty2($qty,$p_id){
    global $db;
    $qty = (int) $qty;
    $id  = (int)$p_id;
    $sql = "UPDATE products SET quantity=quantity -'{$qty}' WHERE id = '{$id}'";
...
function find_all_ventas(){
   ...
   $sql  = "SELECT *";
   $sql .= " FROM ventas v";
   $sql .= " LEFT JOIN clientes c ON v.idCliente = c.idCliente";
